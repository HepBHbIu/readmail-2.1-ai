...
- document_number — номер документа реализации/поступления: рядом с «накладная/УПД/реализация/счёт-фактура №», «по документу №», «поступление №». Часто в ТЕЛЕ письма («УПД №83904 от 15 июня»). НЕ номер заявки/претензии/акта/возврата/сумма/дата. ВАЖНО: №Э…/№В… из ТЕМЫ (напр. №Э00022168) — это return_number, НЕ document_number; document_number ищи отдельно по «УПД/накладная №». Если в ТЕЛЕ есть «при приёмке по документу №X» / «по документу №X от ДАТА» — это document_number=X (документ реализации/приёмки), он ПРИОРИТЕТНЕЕ длинного номера УПД из ТЕМЫ (напр. тема «УПД 0006877422», тело «по документу №83816» → document_number=83816).
- document_date — дата документа: «№X от ДАТА» или дата рядом со счётом-фактурой/поступлением (может стоять ПЕРЕД номером). Верни DD.MM.YYYY. Форматы: ISO 2026-06-09→09.06.2026; СЛОВАМИ «5 июня 2026»→05.06.2026, «03 июня 2020 г.»→03.06.2020; кривой «от 2026 ... 06.03» по контексту номера→03.06.2026. ВСЕГДА ищи дату рядом с № документа, даже если формат неаккуратный или дата словами. ПРИМЕР: «83245 от 05 июня 2026 Отказ» → document_number=83245, document_date=05.06.2026 (дата словами сразу после «от»). НЕ дата письма/заявки.
...
- marking_request — маркировка/ТНВЭД/ЭДО. ВАЖНО: если ПРИЧИНА именно маркировка («Отсутствует/Повреждена контрольная марка», «Честный Знак/ЧЗ не в обороте/не сходится дата», «нет кода/ДатаМатрикс», «расхождение по дате с ЧЗ», «не смогли расшифровать дату» в контексте ЧЗ) → event_type=marking_request, claim_kind=marking_request ДАЖЕ при словах «возврат»/«брак» рядом (ЧЗ/маркировка важнее слова «брак»). НО: просто ФОТО детали/упаковки или слово «маркировка» о наклейке на товаре (без ЧЗ/контрольной марки/ДатаМатрикс/ТНВЭД) — это НЕ marking_request, а обычный возврат/брак. И «фото маркировки ПРОИЗВОДИТЕЛЯ на изделии», «направьте фото маркировки» — это ТРЕБОВАНИЕ ДОКАЗАТЕЛЬСТВА для рекламации (defect/возврат), НЕ marking_request.
...
ВАЖНО (анти-ложный-мусор): если в письме ЕСТЬ № документа + артикул + причина возврата — это new_return, ДАЖЕ если тема выглядит служебной («почтовый робот», «заявка на доставку», «автоматическое уведомление»). Полнота возвратных данных важнее «робот»-вида отправителя.
