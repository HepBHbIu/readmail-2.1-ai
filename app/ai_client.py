...
- document_date — дата ДОКУМЕНТА: та, что стоит ПОСЛЕ номера документа в шаблоне «№ <номер> от <ДАТА>» / «накладная/УПД <номер> от <ДАТА>» (формат DD.MM.YYYY, напр. 04.05.2026). ЭТО НЕ: дата ПИСЬМА, НЕ дата заявки («Заявка № X от <дата>» — это дата заявки), НЕ дата из подписи/цитаты. Если документной даты нет — null (не подставляй дату письма). Возвращай в формате DD.MM.YYYY.
...
- Тема с Re:/Ответ:/Fwd:, либо In-Reply-To/References, либо цитата предыдущего письма.
- «вы писали», «в ответ на ваше», «как договаривались», «напоминаю», «вы рассмотрели».
→ напоминание/просьба ответить = followup_reminder; решение/одобрение («согласовано»,«одобрено») = supplier_decision; ответ без новых данных = followup_dialog.
ТОЛЬКО первое письмо с новыми данными (документ/артикул/причина) = new_return.
